...
| Write actions | PoE cycle, AP reboot, PF reevaluate/restart, camera snapshot | **all four live** (`/api/actions`, audited) | ✅ D4 built 2026-07-29 — ZCD parity reached; camera snapshot is D7, separate |
...
| D4 | Operator write actions (PoE cycle via rConfig, XIQ AP reboot, PF reevaluate-access / restart-switchport) behind operator/admin role + audit log + per-action config flag (default off) | Approve as post-cutover phase (11.x), default-disabled; until then disabled buttons with "managed in <source>" tooltips | ✅ **approved 2026-07-28** — design signed off (operator/admin role + audit log + per-action flag, default off); **build in 11.x, post-cutover**. Read-only-first holds through Phase 8; disabled buttons with "managed in <source>" tooltips until then **BUILT 2026-07-29, ahead of the planned 11.x slot, at the owner's direction.** Two deviations from this signed design, both owner-chosen and recorded rather than silent: flags default **true** ("live on merge") instead of default-off with a dry-run first (§4.2), and the role floor is `operator`. Delivered as `netmon/actions.py` (closed action registry + audited chokepoint), `netmon/api/actions.py` (4 POSTs + capability advertisement + audit read), migration `020_action_audit`, and buttons on Switches port-detail / AP Detail / PacketFence. Targets resolve server-side from the registry — a caller sends a `device_id`, never a URL — and a PoE cycle verifies the port exists on that switch, because the rConfig snippet is trusted but its arguments are not. |
