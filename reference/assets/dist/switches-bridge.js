...
(function () {
  ...
  window.tcsNavigateSwitch = function (hostid) {
    ...
    window.SWITCH_LOADING = {
      ...window.SWITCH_LOADING,
      snapshot: true
    };
    window.dispatchEvent(new CustomEvent("tcs:switch-data", {
      detail: {
        section: "navigate"
      }
    }));
  ...
  window.makePortDetail = function (memberIdx, port) {
    ...
    const devices = pfRows.map(_toDevice).sort((a, b) => _seenMs(b.lastSeen) - _seenMs(a.lastSeen));
    ...
    return {
      ...
      device,
      devices,
      extraMacs: device ? Math.max(pfRows.length - 1, macs.length > 1 ? macs.length - 1 : 0) : macs.length > 1 ? macs.length - 1 : 0,
      macs,
  ...
  function buildStack(members, ports, poe) {
    ...
    const memberIdxs = members.length ? members.map(m => Number(m.index)).filter(n => n > 0) : [...byMember.keys()].sort((a, b) => a - b);
    const stack = [];
  ...
  function applyFleet(fleet) {
    ...
    const matches = sw => activeHostid !== "" ? String(sw.hostid) === activeHostid : sw.id === activeHost;
  ...
  function fetchSnapshot(switchid) {
    ...
    }).catch(e => console.error("[tcs] snapshot fetch failed:", e, "url:", url)).finally(() => {
  ...
  fetchJson(URL_FLEET + "&mode=skeleton").then(j => {
    const fleet = Array.isArray(j && j.fleet) ? j.fleet : [];
    applyFleet(fleet);
  }).catch(e => console.error("[tcs] fleet skeleton fetch failed:", e)).finally(() => {
    window.SWITCH_LOADING.fleet = false;
    notify("fleet");
  });
  fetchJson(URL_FLEET + "&mode=counters").then(j => {
  ...
  window.tcsLoadPortHistory = async function (member, port) {
    ...
    if (!hostid || !member || !port) return {
      inHist: FLAT60,
      outHist: FLAT60
    };
    const url = `${URL_PORTHIST}&hostid=${encodeURIComponent(hostid)}` + `&member=${encodeURIComponent(member)}&port=${encodeURIComponent(port)}`;
    try {
  ...
  window.tcsPfDeviceAction = async function (mac, op) {
    ...
    const hostid = window.TCS_SWITCH_HOSTID;
    if (!url || !hostid) return {
      ok: false,
      error: "endpoint not configured"
    };
    if (!mac || !op) return {
      ok: false,
      error: "mac and op required"
    };
    try {
