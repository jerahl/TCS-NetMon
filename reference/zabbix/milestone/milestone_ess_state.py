...
# websockets >= 13 ships a new asyncio client at websockets.asyncio.client.connect
# that takes `additional_headers`. The legacy top-level websockets.connect (still
# the only client in <13, and still present in 13+ for back-compat) takes
# `extra_headers`. Bind to whichever is available at import time — a runtime
# try/except around the call doesn't work, because kwargs are forwarded as
# **kwargs and only blow up later inside loop.create_connection() (cron log:
...
async def fetch_state(
    ...
    # Step 2: WSS. The headers-kwarg name (extra_headers vs additional_headers)
    # is resolved at import time via _WS_HEADERS_KW / _ws_connect above —
    # websockets >= 13 wants the new asyncio client + additional_headers, and
    # the legacy top-level connect still uses extra_headers.
    #
    ...
    ctx = _ws_connect(ws_url, **ws_kwargs)
