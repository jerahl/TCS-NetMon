...
def _ap_uplink(engine: Engine, detail: dict, pf: dict | None = None) -> dict | None:
    """An AP's access port. The resolution — and the trunk-avoidance that makes
    it safe to act on — lives in netmon.uplink, shared with camera detail."""
    return uplink_for_mac(engine, (detail or {}).get("mgmt_mac"), pf)
